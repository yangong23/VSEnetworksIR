...
def itm_eval(score_matrix, txt_ids, img_ids, txt2img, img2txts):
    ...
    _, rank_img = score_matrix.topk(20, dim=0)
    tr_r1, tr_r5, tr_r10, tr_r20 = 0, 0, 0, 0
    for j, img_id in enumerate(img_ids):
        gt_is = [txt2i[t] for t in img2txts[img_id]]
        ranks = [(rank_img[:, j] == i).nonzero() for i in gt_is]
        rank = min([20] + [r.item() for r in ranks if r.numel()])
        if rank < 1:
            tr_r1 += 1
        if rank < 5:
            tr_r5 += 1
    ...
    eval_log = {'txt_r1': tr_r1,
                ...
                'img_r_mean': ir_mean,
                'r_mean': r_mean}
    return eval_log, Retrievaltime


@torch.no_grad()
def evaluate(model, eval_loader, stime):
    st = time()
    ...
    eval_log, Retrievaltime = itm_eval(all_score, all_txt_ids, all_img_ids,
                        dset.txt2img, dset.img2txts)

    tot_time = time()-st
    t1 = Retrievaltime - stime
    t2 = Retrievaltime - st
    print("time1:%.2f" % t1)  # 结束时间-开始时间
    print("time2:%.2f" % t2)  # 结束时间-开始时间
