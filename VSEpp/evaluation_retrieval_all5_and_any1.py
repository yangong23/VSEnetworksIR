...
# Reference: The code is modified based on VSE++: Improving Visual-Semantic Embeddings with Hard Negatives (https://arxiv.org/abs/1707.05612). 
# Program evaluates VSE++ for retrieving all 5 descriptions to the image query.
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        print("Image to text: @5:%.1f @10:%.1f @20:%.1f %.1f %.1f" % r)
...
def i2t(images, captions, npts=None, measure='cosine', return_ranks=False):
    ...
    AveF1_Measure_temp = numpy.zeros(images.shape[0])
    ...
    for index in range(npts):
        ...
        for j in range(len(inds_ID)):
            ...
            # F1-Measure
            if Recall_temp == 0 and Precision_temp == 0:
                F1_Measure_temp = 0
            else:
                F1_Measure_temp = 2 * Recall_temp * Precision_temp / (Recall_temp + Precision_temp)
            F1_Measure.append(F1_Measure_temp)
    ...
    # Compute metrics
    AveRecall = AveRecall_temp / npts
    AvePrecision = AvePrecision_temp / npts
    AveF1_Measure = AveF1_Measure_temp / npts

    ar = []
    for q in range(5000):
        q = q + 1
        rt = 100.0 * len(numpy.where(ranks < q)[0]) / len(ranks)
        ar.append(rt)

    dataframe = pd.DataFrame({'@K': AtK, 'Average Recall RetrievalAll5': AveRecall, 'Average Precision RetrievalAll5': AvePrecision,
    ...
    meanr = ranks.mean() + 1
    if return_ranks:
        return (AveRecall[4], AveRecall[9], AveRecall[19], medr, meanr), (ranks, top1)
    else:
        return (AveRecall[4], AveRecall[9], AveRecall[19], medr, meanr)
