...
def func_attention(query, context, opt, smooth, eps=1e-8):
    ...
    # Get attention
    # --> (batch, d, queryL)
    queryT = torch.transpose(query, 1, 2) # Transposition

    # (batch, sourceL, d)(batch, d, queryL)
    # --> (batch, sourceL, queryL)
    # sij
    attn = torch.bmm(context, queryT)#torch.bmm () is a multiplication operation in tensor, similar to a * B in matrix.
    ...
    attn = nn.Softmax()(attn*smooth) # aij
...
def xattn_score_i2t(images, captions, cap_lens, opt):
    ...
    for i in range(n_caption):
        ...
        # (n_image, n_word, d)
        cap_i_expand = cap_i.repeat(n_image, 1, 1) # repeat 128 times
        """
            word(query): (n_image, n_word, d)
