...
# Program analyses the performance of VSRN on ImageNet classes and gets the retrieval details for all image queries (task of image-to-text retrieval).
...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        r, rt = i2t(img_embs, cap_embs, measure=opt.measure, return_ranks=True, mode = split)
...
def i2t(images, captions, npts=None, measure='cosine', return_ranks=False, mode = 'test'):
    """
    ...
    AveRecall_temp = numpy.zeros(images.shape[0])
    AvePrecision_temp = numpy.zeros(images.shape[0])
    AtK = []
    ...
    # mode = 'train'
    NumTopics = 1000
    ImageTopicID = np.loadtxt(sourcePath  + 'ImageNet_' + mode + 'ImageTopicID.txt', dtype=int)
    ImageTopicNum = np.loadtxt(sourcePath  + 'ImageNet_' + mode + 'TopicImageNum.txt', dtype=int)
    ...
    caps = []
    if mode == 'test':
        df = pd.read_csv(sourcePath + 'test_img_ID_name.csv')
        with open('source/test_caps.txt', 'r', encoding='utf-8') as f:
                for line in f.readlines():
                    caps.append(line)
    elif mode == 'dev':
        df = pd.read_csv(sourcePath + 'dev_img_ID_name.csv')
        with open('source/dev_caps.txt', 'r', encoding='utf-8') as f:
                for line in f.readlines():
                    caps.append(line)
    ...
    AveRecallPerTopic = [np.zeros(tLen) for i in range(NumTopics)]
    AvePrecisionPerTopic = [np.zeros(tLen) for i in range(NumTopics)]
    ...
    for index in tqdm(range(npts)):
        ...
        for j in range(1000):
            inds_ID_s_temp = '%s'%inds_ID[j] + '_' + '%s'%inds_ID_d[j]
            inds_ID_s.append(inds_ID_s_temp)
            ...
            AveRecallPerTopic[ImageTopicID[index]][j] = AveRecallPerTopic[ImageTopicID[index]][j] + Recall_temp
            ...
            AvePrecisionPerTopic[ImageTopicID[index]][j] = AvePrecisionPerTopic[ImageTopicID[index]][j] + Precision_temp
    ...
    AveRecallPerTopic_r5 = []
    AvePrecisionPerTopic_p1 = []

    for i in range(NumTopics):

        if ImageTopicNum[i] == 0:
            Inum = 1
        else:
            Inum = ImageTopicNum[i]

        AveRecallPerTopic[i] = AveRecallPerTopic[i] / Inum
        AvePrecisionPerTopic[i] = AvePrecisionPerTopic[i] / Inum
        AveRecallPerTopic_r5.append(AveRecallPerTopic[i][4])
